...
describeWithEnv('audit ingestion over a real RabbitMQ (real PostgreSQL 16)', ['TEST_DATABASE_ADMIN_URL', 'TEST_RABBITMQ_URL'], (env) => {
  ...
  it('events of every catalog producer are admitted when their source is the action\'s owner (all 53 bus actions; release-service since Stages 20.3 / 20.4); audit-service\'s own action never over the bus', async () => {
    await start();
