...
/** Stage 18.6: the organization / platform audit reads (service token + `AUDIT_SERVICE_POLICY`; the kit's Postgres rate limiter). */
@Module({
  imports: [PersistenceModule, RateLimitModule],
  controllers: [AuditQueryController],
  providers: [{ provide: QueryCounters, useValue: new QueryCounters() }, AuditQueryService, QueryReporter],
