...
export async function createTestApp(opts: { databaseUrl?: string; env?: NodeJS.ProcessEnv; migrationsDirs?: string[] } = {}): Promise<TestApp> {
  ...
  const moduleRef = await Test.createTestingModule({ imports: [AppModule.register(config, { migrationsDirs: opts.migrationsDirs })] })
    .setLogger(logger)
    .compile();
