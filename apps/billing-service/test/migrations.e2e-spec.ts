...
const BILLING_MIGRATIONS = [
  ...
  '0005_invoice_number_sequence.sql', '0006_payment_request.sql', '0007_payment_event_receipt.sql', '0008_billing_transition.sql',
];
/** The Stage 2 schema, exactly (SDD 28 and 34.1). The exact-set assertions are the tripwire against a table for a DEFERRED concept. */
const STAGE_2_TABLES = [
  'billing_transition', 'currency', 'inbox', 'invoice', 'invoice_line', 'invoice_number_sequence', 'kit_rate_limit', 'outbox',
  'payment_event_receipt', 'payment_request', 'price', 'product', 'schema_migrations',
];
...
describeWithEnv('migration infrastructure (real PostgreSQL)', ['TEST_DATABASE_ADMIN_URL'], (env) => {
  ...
  it('applies the kit migrations and the eight Billing migrations from an empty database, in order, and creates exactly the Stage 2 tables', async () => {
  ...
  it('the service migrations folder holds exactly the eight migrations the SDD lists, and none is edited in place afterwards (checksums)', async () => {
