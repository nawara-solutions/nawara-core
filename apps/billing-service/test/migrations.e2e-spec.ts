...
describeWithEnv('migration infrastructure (real PostgreSQL)', ['TEST_DATABASE_ADMIN_URL'], (env) => {
  ...
  it('applies the kit migrations and the nine Billing migrations from an empty database, in order, and creates exactly the Stage 2 tables', async () => {
  ...
  it('the service migrations folder holds exactly the nine Stage 2 migrations, and none is edited in place afterwards (checksums)', async () => {
