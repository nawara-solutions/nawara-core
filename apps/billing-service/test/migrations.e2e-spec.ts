...
const BILLING_MIGRATIONS = [
  ...
  '0010_product_producer.sql',
];
/** The Stage 2/3 schema, exactly (SDD 28 and 34.1; migration 0010 adds a column, not a table). The exact-set assertions are the tripwire against a table for a DEFERRED concept. */
...
describeWithEnv('migration infrastructure (real PostgreSQL)', ['TEST_DATABASE_ADMIN_URL'], (env) => {
  ...
  it('applies the kit migrations and the ten Billing migrations from an empty database, in order, and creates exactly the Stage 2/3 tables', async () => {
  ...
  it('the service migrations folder holds exactly the ten Stage 2/3 migrations, and none is edited in place afterwards (checksums)', async () => {
