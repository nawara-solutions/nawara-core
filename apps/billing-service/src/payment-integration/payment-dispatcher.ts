...
export class PaymentDispatcher {
  ...
  private readonly loop = new PollLoop(() => this.dispatchOnce(), (e) => this.logger.error(`payment_dispatch_pass_failure error=${e instanceof Error ? e.name : 'unknown'} — the next pass retries`));
