 ...
 */
export function createEventBus(config: Pick<PaymentConfig, 'rabbitmqUrl' | 'isProduction'>): EventBus {
  if (config.rabbitmqUrl) return new RabbitMqEventBus({ url: config.rabbitmqUrl });
