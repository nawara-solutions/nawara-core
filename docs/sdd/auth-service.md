...
- `User.secretKeyHash`/`User.secretKeyIssuedAt` (per ADR-0010) exist only for owners — one
  ...
  both fields atomically, instantly invalidating the old key.
...
- `User.isActive` is no longer just schema-only groundwork: as of ADR-0012, the new
  ...
  write it (`false`/`true` respectively). It remains generically named and generically
  checked at login/refresh (see Error handling & edge cases) — nothing about it is
  operator-specific at the schema level, only the endpoints that write it currently are.
