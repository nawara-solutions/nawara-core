...
| R-2 | The architecture's `product.type` (`one_time`, `recurring`, `organization_license`, `user_subscription`) mixes *billing interval* with *entitlement kind*. | Split as designed: the **interval belongs to the price** (`price.interval`, built and load-bearing). The **entitlement-kind split** (`product.entitlementKind`) was also built (migrations `0002`, `0004`) but **turned out not to be what drives Subscription behaviour** — Stage 12.4's `linkSubscription` keys off `price.interval = 'recurring'` alone, never reads `product.entitlementKind` at all. The column and its CHECK constraint (`none`\|`organization_license`\|`user_subscription`) still exist and still accept input. It has **no commercial decision-making reader** (it drives neither Subscription, `deriveEntitlement()` nor Payment settlement), but it is **not unused**: it remains a compatibility field on the Product API, the product replay identity, the invoice-line snapshot and the `invoice.created` payload. Its retention and any future retirement are recorded in [ADR-0045](../adr/0045-commercial-entitlementkind-compatibility-fields.md). |
...
| R-12 | **Two unrelated "templates".** `core-architecture.md` gives notification-service "templates" (message templates for email, SMS, push). An *invoice template* (presentation of a financial document) is a different concept. | Invoice templates belong to Billing (section 36); notification-service keeps its message templates. The name is never shared between the two. |
...
| BI-07 | An invoice total cannot become inconsistent with its lines after creation | `invoice_line` rows cannot be inserted into a non-`draft` invoice, nor updated or deleted ever; `invoice` and `invoice_line` are **immutable by default**: a Billing-local trigger refuses a change to **any column not on an explicit mutable allow-list** (`status`, `revision`, `updatedAt`, `number`, `issuedAt`, `presentation` once, `paidAt`, `voidedAt`, `voidReasonCode`, `overdueAt`), so a column added by a later migration is immutable until someone deliberately allows it. The kit's `forbid_column_change` is a **deny-list** (a forgotten column would be silently mutable) and is not used for these two tables | no update path exists | DB: UPDATE/DELETE of a line, UPDATE of totals, INSERT of a line into an `open` invoice all refused |
...
| BI-15 | A payment request's amount, currency and mapped fields never change after insert | `forbid_column_change` on `payment_request` (`invoiceId`, `amount`, `currency`, `expiresAt`, `mappingVersion`, `createdAt`, `createdByType`, `createdById`, `correlationId`) | none | DB: UPDATE refused |
...
| BI-20 | **Party and presentation snapshots are historical truth.** `issuerSnapshot` and `billToSnapshot` never change after creation; `presentation` is NULL in a `draft`, set exactly once at issue, and never changes; none of them can alter an amount, a currency or a status | the allow-list trigger of BI-07; CHECK `issuedAt IS NULL OR presentation IS NOT NULL`; CHECKs that each is a JSON object with a `schemaVersion` and at most 8 KB | built server-side; a renderer only reads them | DB: UPDATE of each snapshot refused; a second `presentation` write refused; issuing without a presentation refused |
| BI-21 | **The currency reference is immutable.** A currency's `exponent` never changes and a referenced row is never deleted: changing an exponent would silently re-scale every historical amount | trigger refusing UPDATE of `exponent` and DELETE on `currency`; the FKs already protect referenced rows | rows are seeded by migration only | DB: exponent UPDATE and row DELETE refused |
...
| `issuerSnapshot`, `billToSnapshot` | JSON objects (`jsonb`, at most 8 KB each, a `schemaVersion` inside), **immutable from creation** | The historical **legal and display identity** of the two parties as they were when the obligation was created: names, address lines, tax and registration identifiers, all **opaque strings** that Billing stores and never interprets. **Supplied by the producer** (B-029) until a Billing Profile exists (36.2); **never authority** for payer, seller or organization (those stay the typed ids). Mandatory content and formats are **B-007**; Stage 2 fixes only the container and its immutability. Kept apart from every financial column so personal data can be governed on its own (B-032) |
| `presentation` | JSON object (`jsonb`, at most 8 KB, a `schemaVersion` inside), **set exactly once, at the issue transition**; NULL in a `draft` | The **presentation snapshot**: which immutable presentation definition and locale the invoice was issued with (section 36). Present in every invoice that has been issued. It never holds an amount |
...
| `entitlementKind` | `NOT NULL` (migration `0004`), copied from `product.entitlementKind` at line creation and append-only like the rest of the line. **No commercial decision-making reader** — corrects this row's original claim that it drives an entitlement decision (R-2/section 15): the actual mechanism is the `interval` column above. It is **not unused**: it is part of the persisted line snapshot and is published in the `invoice.created` payload (section 23), so it is retained for event/snapshot compatibility (ADR-0045). It is not in the invoice HTTP representation |
| `quantity` | integer, `>= 1` |
...
| Fields | `id`, `seller{type,id}`, `code` (`^[a-z][a-z0-9_-]{1,62}$`), `name` (at most 140), `description` (at most 280, nullable), `entitlementKind` (`none` \| `organization_license` \| `user_subscription`, default `none`, immutable — a **compatibility field, not commercial authority**: Subscription-linking (section 15/21.4) keys off `price.interval = 'recurring'` alone, and neither `deriveEntitlement()` nor Payment settlement reads it. It still participates in the Product API (input and output), the create-product replay identity (a different value for the same `(seller, code)` is `409 product_conflict`) and the invoice-line snapshot; see R-2 and ADR-0045), `status` (`active` \| `archived`), `revision`, timestamps |
...
| 8 | `GET /billing/invoices/{id}` | service token or user bearer | n/a | 3 (producer and payer relations, both `[T]`, 19.3); organization relations **B-027** |
| 9 | `GET /billing/invoices` | service token or user bearer | n/a | 3 (same as endpoint 8); organization-member listing **B-027** |
...
| 13 | `POST /billing/invoices/{id}/payment-requests` | user bearer or service token | by state (BI-13) | 3 — **creation only**: a durable Billing-side record, never sent to Payment (that is Stage 4's dispatcher, 21.5); org payers **B-026** |
...
* **DLQ handling:** a Payment event whose processing throws is retried a bounded number of times (transient errors only) and then held in `billing.payment-events.dead`: an operational alert. Replaying it is safe (`payment_event_receipt` de-duplicates) and goes through the same consumer; the procedure is the `nawara-dlq` runbook in `apps/billing-service/README.md`. `conflict` and `deferred` outcomes are acknowledged and recorded in their receipt, they are not dead-lettered.
...
**Common payload** (every invoice event): `invoiceId`, `invoiceNumber`, `sourceType`, `sourceId`, `organizationId`, `payer{type,id}`, `seller{type,id}`, `currency`, `subtotal`, `taxTotal`, `total`, `taxTreatment`, `status`, `revision`, `dueAt`, `issuedAt`, `actor{type,id}` (`user`, `service`, `system`), `cause{type,id}` (the request, payment event, sweep, reconciliation or dispatcher), and `aggregateType: "invoice"`. The correlation id is in the header; **system-initiated** events set it to the causing event id, or — for the dispatcher and reconciler (`jobTransitionContext`, Stage 5 hardening) — to the payment request's own originating correlation id if it was created under one, falling back to a deterministic per-request id (`dispatch:<id>` / `reconcile:<id>`) otherwise, so **every event has one** and, where possible, the SAME one across the whole HTTP-request → dispatch → Payment → event → consumer loop, not just a fresh id per job run. Amounts are JSON integers. Version `1` for all events below.
...
The kit provides structured logs with request and correlation ids and redaction, `/health` and `/ready`; it provides **no metrics facility** (Payment lists the same gap in its README, "Known limitations"). Required signals (**counters/a dashboard are still Stage 9, and the metrics mechanism itself remains a prerequisite decision**): counters of invoices by status; payment requests by status; **payment events by receipt outcome** (`applied`, `ignored`, `conflict`, `deferred`); dispatcher sends, retries and `rejected`; reconciler settlements; outbox lag and unpublished age; DLQ depth; overdue sweep lag; rate-limit rejections. **Alerts:** any `conflict` or `rejected`, and a `deferred` receipt not settled within a threshold; a request `sending` or `requested` longer than a threshold; a dead-lettered event; outbox age. Until a metrics mechanism exists these are structured log lines with stable names, not silent.

**Implemented (Stage 5 hardening, log-only — not a metrics mechanism):** `PaymentDispatcher`, `PaymentReconciler` and `PaymentEventConsumer` now emit one structured log line per stable outcome name — `payment_dispatch_success`, `payment_dispatch_rejected`, `payment_dispatch_failure`, `payment_dispatch_stale_recovery`, `payment_reconcile_success`, `payment_reconcile_failure`, `payment_reconcile_deferred`, `payment_event_applied`, `payment_event_ignored`, `payment_event_conflict`, `payment_event_deferred`, `payment_event_dead_letter` — each carrying the request/event id and the correlation id (21.5, 21.4). An unpublished outbox row now logs `outbox_publish_failure` instead of failing silently (the kit's `EventsModule` `onError` hook, previously unwired). DLQ depth is checked on demand with `nawara-check-dlq --queue billing.payment-events.dead` (`libs/service-kit/src/cli/check-dlq-depth.ts`), a plain queue-depth read, not a metrics platform. Unpublished-outbox count and oldest-pending age are checked the same way with `nawara-check-outbox-lag --database-url <url> [--max-age-seconds 60]` (`libs/service-kit/src/cli/check-outbox-lag.ts`) — a read against the same `outbox` table `OutboxRelay` already claims from, exiting non-zero only once the oldest unpublished row is older than the threshold (a fresh in-flight row is normal and must not trip it). Neither CLI retries, republishes or deletes anything. Counters and alerting on these signals are still Stage 9 work, gated on the same metrics-mechanism decision as before.
...
| B-004 | **Invoice numbering:** scope (seller, legal entity, platform), format, series, reset, gapless requirement; when a fiscal year is part of the scope, the year is taken in the **issuer's time zone**, not UTC (B-007) | legal validity, uniqueness | `invoice.number`, `invoice_number_sequence` · 10 | the **format**; the counter mechanism and the provisional uniqueness proceed but are not for real customers |
...
| B-007 | **Legal invoice requirements:** mandatory issuer and customer identity content in the snapshots, language, the **issue-date and due-date semantics** (a calendar date in the issuer's time zone, not a UTC instant), whether the rendered document is itself the legal original, retention | legality of issuing | `invoice` fields, presentation · events | issuing to real customers |
...
| **2 Schema and invariants** | migrations `0001` to `0008` with every trigger and CHECK of section 8 (including BI-20, BI-21 and the mutable-column allow-list, and the snapshot columns of section 9); the SQL invariant suite (one constraint per assertion); the TypeScript-vs-trigger agreement test; concurrency races in SQL (issue numbers, one active request) | Stage 1 |
| **3 Catalog and invoices** | products, prices, create/get/list/issue/discard invoices, natural-key idempotency, totals computed server-side, events (`invoice.created`) through the outbox, with the **test producer fixture**; authorization for the producer **and**, since the relation is already `[T]` (19.3) rather than `[B]`, the `user` payer — invoice read and list (endpoints 8, 9), with their own collapsed-`404`/IDOR coverage (unrelated user, other producer, an Auth admin/owner flag granting nothing). Also creates Billing's own `payment_request` record (endpoint 13: BI-13 state idempotency, amount/currency always the invoice's) — **record only, never sent to Payment**; `paymentId` stays `null` and `status` stays `created` until Stage 4's dispatcher exists. Organization-member relations remain blocked (B-027) | Stage 2 |
...
6. Accept the pre-Stage 2 audit amendments before Stage 2 starts: the snapshot columns of section 9, BI-20, BI-21, the mutable-column allow-list under BI-07, and the boundaries of section 36 (R-11, R-12). No new business decision was added; B-004, B-007 and B-032 were widened.

## 36. Presentation, documents and future capabilities (pre-Stage 2 audit amendment)

Added by the pre-Stage 2 architecture audit. **Only the snapshot columns and invariants named in section 9 and BI-20/BI-21 are Stage 2 work.** Everything else here is **[X]**: it records the boundary so a later feature cannot force a redesign of the financial core. The existing invoice states, events and BI-01 to BI-19 are unchanged.

### 36.1 Ownership of each concept

| Concept | Is | Owner |
|---|---|---|
| Invoice (with lines, totals, currency, party snapshots) | **financial truth** | Billing |
| Invoice Template | presentation configuration of an organization | Billing (future, **[X]**) |
| Template Version | an **immutable** published presentation definition | Billing (future, **[X]**) |
| Invoice Presentation Snapshot (`invoice.presentation`) | which template version and locale this invoice was issued with | Billing (Stage 2 column) |
| Invoice Document | a rendered artifact of one invoice, with its own metadata | Billing metadata (future, **[X]**) |
| File bytes, checksums, access control, lifecycle | stored files | file-service (planned, does not exist) |
| Delivery (email, notification, secure link message) | delivery | notification-service (planned starter) |
| Central event history | who did what and when | audit-service (planned, does not exist) |
| Money movement | payment | payment-service |
| Financial effect, tax ledger | accounting | accounting-service (planned, does not exist) |

Rules: **a template controls presentation, never a financial value** (it may hide a tax column; it can never set a tax amount); a renderer **reads** stored invoice values and never computes, rounds or converts money; a presentation snapshot never holds an amount; Billing never stores file bytes.

### 36.2 Parties

| Concept | Classification | Position |
|---|---|---|
| Organization | [D] | hierarchy entity (Auth today, organization-service intended, ADR-0031); opaque here |
| Seller | [D] | the typed party that is paid (ADR-0036); the Payment contract limits its type to `user`, `organization`, `company` |
| Legal issuer | **[B]** (B-001, B-002) | whose legal identity appears. The design does **not** assume "organization = legal issuer": the invoice carries an **issuer snapshot** (identity as data), so a later legal-entity concept attaches without changing the invoice |
| Payer / customer | [D] | the typed party that owes and is charged (`payerType`, `payerId`); in v1 customer = payer. Owner and pays-on-behalf is B-026 (Payment O-18) |
| Bill-to party | [T] | the **`billToSnapshot`** of the payer (display and legal identity). Not authority |
| External customer with no Nawara identity | [X] | Payment's party types are fixed (ADR-0036); a new type needs a decision in Billing **and** Payment |
| Billing Profile | [X] | a future Billing-owned record **of invoicing-purpose settings and overrides** (invoicing legal name if different, payment instructions, default currency, invoice preferences, branding references, timezone). It does **not** own organization identity facts (R-11). Additive: invoices already snapshot, so no Stage 2 table is needed |
| Multiple legal entities, branches, trade name vs legal name | [X] | expressed as several Billing Profiles of one seller plus the issuer snapshot; numbering scope per B-004. No Legal Entity Service is created |

### 36.3 What Stage 2 includes and what it defers

| Item | Stage 2 |
|---|---|
| `issuerSnapshot`, `billToSnapshot` (immutable from creation), `presentation` (set once at issue), BI-20, BI-21, the mutable-column allow-list | **include** (a snapshot cannot be backfilled truthfully after invoices exist) |
| Content and mandatory fields of the snapshots, locale allow-list, the built-in default presentation (`system:1`, frozen in code) | **defer** (B-007 and the designer); Stage 2 fixes only container, size, `schemaVersion` and immutability |
| Billing Profile, Invoice Template, Template Version, Invoice Document tables | **defer** (additive) |
| `customerReference`/purchase-order number, invoice `notes`, per-line service period (`periodStart`, `periodEnd`), per-line tax label and rate, a `verificationRef` | **defer**: additive nullable columns of `invoice`/`invoice_line`; the allow-list trigger makes them immutable by default when added |
| Discounts | **defer** (B-033); BI-05 and the line-total CHECK change with that decision, as already recorded |

### 36.4 Invoice Template Designer boundary [X]

Not implemented and not scheduled. The designer edits **presentation only**, through a **controlled block editor**: an organization enables, orders, labels and styles the blocks Billing supports (header, customer, invoice information, items, discount, tax, totals, payment instructions, notes, QR or verification, footer) within Billing's allowed ranges. It never edits code.

**Storage model (direction, for when it is built): hybrid.** A relational row holds identity and lifecycle (template id, owner, version number, status, publishedAt, immutable once published); the block tree is a **validated, closed-vocabulary JSON `definition` with a `schemaVersion`**. Reasons: the tree is edited, validated, rendered and versioned as a unit and is never queried by inner field; fully relational block tables would need a migration for every new block property and give no query benefit; a closed JSON schema (no unknown keys, enumerated values, bounded sizes) gives the security property that matters (no executable content) and versioned upgraders give compatibility. A relational-only or free-form JSON store is rejected for those reasons.

**Versioning.** A published template version is immutable; editing creates a new version; an invoice issued with version *n* keeps referencing it forever (its `presentation` names it). A template is `draft` (editable, never used for issuing) or `published` versions.

**Security constraints (binding on the future designer):** no scripts, no expressions, no raw HTML or CSS (styles are enumerated tokens: validated colours, allow-listed fonts, bounded sizes and margins); all text is escaped; assets (logo) are **File Service ids owned by the same organization**, checked at publish and at render (no cross-organization access) and size- and type-limited; the renderer has **no network access**, runs with time and memory limits, and rejects oversized definitions (no resource exhaustion, no SSRF).

### 36.5 Documents, rendering and PDF [X]

* **Flow:** authoritative invoice + presentation snapshot (+ its template version) → `InvoiceRenderer` → bytes → **file-service** → an **Invoice Document** record in Billing.
* **Renderer:** an **internal Billing capability behind an `InvoiceRenderer` port**, not a new service; extract it only if scale or isolation later justifies it. Text, dates and numbers are formatted from the locale, and money from the `currency` table's exponent (BI-21).
* **Invoice Document** (Billing metadata): document version, `generatedAt`, template reference, renderer version, content hash, opaque file-service id. **Immutable**; regenerating creates a **new version**, never overwrites. The content hash lets File Service's stored checksum and Billing's record be compared. Whether the rendered document is itself the legal original is B-007. Billing does not become a file store.
* **What Stage 2 preserves so PDFs can be added later:** the complete financial snapshot, both party snapshots, the presentation snapshot, and an immutable currency reference. Nothing else is needed.

### 36.6 Localisation, currency and dates

Languages (Arabic, French, English) and right-to-left layout are **presentation**: an invoice is issued in **one** locale, recorded in `presentation`; line descriptions are snapshotted in that language (any per-language catalogue name is resolved when the invoice is created, never at render time). The producer supplies the locale from a per-deployment allow-list; Billing never guesses one and no language appears in a financial column. Currency is single and immutable per invoice (BI-03), formatted from the exponent; there is **no implicit conversion**, and exchange rates stay **[X]**. Payment consistency is unchanged (13.2).

### 36.7 Delivery, access, verification, integrity and signatures [X]

* **Delivery** (download, email, secure link): notification-service delivers; Billing emits facts and never sends messages.
* **Secure access:** a document is never served by a guessable id. Authenticated routes authorize the caller against the invoice (producer, `user` payer; organization members are B-027) and hand out a **short-lived** File Service URL; access is logged to audit-service. **Public verification** would use a random unguessable `verificationRef`, revocable, exposing only what a verifier needs, never the invoice. No public link exists in v1.
* **QR:** encodes the verification reference only, never invoice data.
* **Integrity and signatures:** the document hash is the hook. **Electronic signature** is a future capability applied to an immutable Invoice Document; no signature service, request, certificate or provider is designed or created.

### 36.8 Audit and history

Locally, Billing keeps the **authoritative** state history (`billing_transition`, `revision`, actor, cause, correlation id; BI-12, BI-19). audit-service (planned) receives Billing's events asynchronously as a **central, queryable, retained** copy, and receives download and access records. Neither can change historical financial truth.

### 36.9 Other documents

Pro-forma invoices, quotations, estimates, receipts and statements are **[X]**. If ever approved, each is its **own entity with its own numbering and no accounting effect**, never an `invoice` state or a `documentType` flag on the invoice table (that would weaken BI-05 to BI-16 for real invoices). A payment receipt is derived from `payment.succeeded`, not from an invoice.
