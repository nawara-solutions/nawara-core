...
| F12 | Checksum | SHA-256 computed by File while streaming; optional client `Content-Digest` compared; provider checksum as a second check (**17.4:** not used, the SDK's automatic checksums are off for portability; carried to the provider evaluation); never authorization or dedupe; **17.8:** re-verified on every download | integrity and corruption detection | trust the client checksum | DECIDED | 17.5 |
...
| F28 | Reconciliation | Bounded workers (lease sweep, orphan expiry, delete retries) + an operator reconciliation tool (missing objects, keyless objects; **17.7:** keyless objects are not searched: no bucket listing); no continuous full-bucket scanner in V1 | cost and simplicity; the states make divergence rare | continuous scanner | DECIDED | 17.7 |
...
| F31 | Timeouts / retries | connect ≤ 2 s, idle-stream 30 s (**17.9:** client idle 30 s, store idle 45 s and enforced), transfer bound from size (**17.9:** downloads too); retries only head / get-before-first-byte / delete | no unbounded SDK defaults (the 16.8 lesson) | SDK defaults | DECIDED | 17.4 |
...
| F36 | Ticket use and leakage | Upload tickets single-use; download tickets reusable until expiry by default (optional single-use; **17.8:** and at most `FILE_TICKET_MAX_DOWNLOADS` uses, default 50); revocable; tokens only in the redemption path, never logged, `Referrer-Policy: no-referrer`, `no-store`; one generic `ticket_invalid` | mobile retries vs replay window kept short | single-use downloads only (breaks retries) | DECIDED | 17.5 / 17.6 |
...
- Upload tickets are single-use; download tickets are reusable until their short expiry (optional single-use; Stage 17.8 added a
  server-controlled use cap, `FILE_TICKET_MAX_DOWNLOADS`); both are revocable (F36). Tokens are never logged.
...
| 17.4 | storage port: filesystem and S3-compatible adapters, bounded timeouts, normalized errors, MinIO adapter tests | before any upload; **done**: [17.4 record](./stage-17-4-storage-abstraction.md) (tests on VersityGW: MinIO no longer publishes public images) |
| 17.5 | upload lifecycle: service streamed upload, size cut-off, magic-byte type check, SHA-256, idempotency, attach, lease sweep; **upload tickets** (issue, single-use redemption) | **moved here:** content validation and checksum belong to upload correctness, not to 17.8; **done**: [17.5 record](./stage-17-5-upload-lifecycle.md) (the lease sweep moved to 17.7 with the other workers) |
| 17.6 | download and authorization: streamed download, safe headers, owner / organization checks, **download tickets** (issue, redeem, revoke, TTL default, redemption rate limits, log redaction) | **done**: [17.6 record](./stage-17-6-download-authorization.md) |
| 17.7 | delete and cleanup: logical delete, delete worker, orphan expiry, reconciliation tool | **done**: [17.7 record](./stage-17-7-delete-cleanup-lifecycle.md) (with the upload-lease sweep, moved from 17.5, and ticket retention) |
| 17.8 | security and integrity: threat-model verification (§6), rate limits, scanner hook (`VERIFYING`), filename / header fuzzing, leak scans | narrowed (validation moved to 17.5); **done**: [17.8 record](./stage-17-8-security-integrity.md) (usage limits, upload bound, reusable-ticket cap, download digest check, name hardening; scanner: decision B, deferred with F19) |
| 17.9 | operational hardening: signals, snapshot, runbooks, key / credential rotation, outbox events if Stage 18 needs them, production image | **done**: [17.9 record](./stage-17-9-operational-hardening.md), [runbook](../../runbooks/file-service.md) (outbox events stay with Stage 18: nothing consumes them yet); **added by 17.8:** measure SHA-256 download-verification CPU cost under concurrency and its throughput impact; download concurrency / connection caps; tune the reusable-ticket cap, the upload concurrency cap and the rate limits ([17.8 record](./stage-17-8-security-integrity.md) §10) |
| 17.10 | focused certification | File only; Stage 22 stays the Core validation; **done**: [17.10 record](./stage-17-10-focused-certification.md) (one 17.9 defect fixed; Stage 17 closure verdict there) |
