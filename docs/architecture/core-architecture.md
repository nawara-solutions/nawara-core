...
| **notification-service** | How is this delivered? | email/SMS/push/in-app delivery, templates, preferences, attempts, retries, providers | why something was triggered | yes (later) | foundation, persistence, event intake, send API (Stage 16.3–16.6) |
...
| notification-service | Foundation (16.3), persistence (16.4) and the Auth event intake (16.5): the queue `notification.events` bound to nine Auth events, each recorded as a durable intent with a PENDING delivery (sealed codes, E.164 enforced); the internal send API (16.6: service token + caller policy, `Idempotency-Key` with a keyed request hash, status and cancel); no worker and no provider yet (nothing is sent). | Nothing until built; it will consume events and expose a token-guarded send API. Design: [ADR-0046](../adr/0046-notification-service-architecture.md) and the [SDD](../sdd/notification-service.md) (Stage 16.1, proposed). |
