...
**`organization-service`** (NestJS/TypeScript) — Company, Platform, Organization. **Implemented (ADR-0039 Stage 9) but NOT yet authoritative:** auth-service still owns these entities and the ownership migration has not occurred (see `apps/organization-service/README.md`). Service-token API only; never owns users or membership.
